 ...
 */
 ...
 //This is the solution for question 14: 
  ...
  /*
    According to the instructions, we need to do the following: 
    ...
    We would also need a while loop. 
    To compare both sorted and unsorted arrays, we would need a for loop that will loop through each element of each array to see if they are the same.
    If it finds a element that is not the same as in the other array, it will break and set a value of 0 to the isTrue variable. 
    If it finds that all elements are the same, it will set a value of 1 to the isTrue variable
    We would also need a counter to count the number of index position in the user-inputted array. 
